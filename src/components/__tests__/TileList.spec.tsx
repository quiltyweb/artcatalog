...
describe("TileList", () => {
  it("renders correctly with default title and alt text", async () => {
    ...
    screen.getAllByAltText(/alt text for testing purposes/i);
  });

  it("renders correctly with fallback alt text for each image", async () => {
    useStaticQuery.mockImplementation(() => ({
      allShopifyCollection: {
        nodes: [
          {
    ...
    screen.getByText("Stickers");
    screen.getByText("Murals & Sign Writing");
    screen.getByAltText(/Products of Original Paintings category/i);
  });
